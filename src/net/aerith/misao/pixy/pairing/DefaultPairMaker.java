...
/**
 * The <code>DefaultPairMaker</code> is a class to make pairs among two
 * lists of stars. From the brightest star to the faintest star, a 
 * star in the first list is getting paired with a star in the second
 * list. If some stars are in the specified search area, the brightest
 * one is selected as a counterpart.
 * <p>
...
public class DefaultPairMaker extends Operation {
	/**
	...
	/**
	 * Constructs a <code>DefaultPairMaker</code> with two lists of 
	 * stars. 
	 * @param first_list  the first list of stars.
	 * @param second_list the second list of stars.
	 */
	public DefaultPairMaker ( StarList first_list, StarList second_list ) {
		...
		throws Exception
	{
		...
		monitor_set.addMessage("[Making pairs]");
		...
		monitor_set.addMessage("Number of pairs: " + pair_list.size());
		monitor_set.addMessage(new Date().toString());
