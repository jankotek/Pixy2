...
/**
 * The <code>RecordSearchInternalFrame</code> represents a frame to 
 * show the XML report documents which contains the specified R.A. and 
 * Decl., and the (x,y) positions.
 *
 * @author  Seiichi Yoshida (comet@aerith.net)
 * @version 2003 August 17
 */

public class RecordSearchInternalFrame extends BaseInternalFrame implements OperationObserver {
	/*
	 ...
	 * The operation.
	 */
	 protected RecordSearchOperation operation;
	...
	/**
	 * Constructs a <code>RecordSearchInternalFrame</code>.
	 * @param infos       the XML information documents.
	 * @param target_star the R.A. and Decl. and radius to search.
	 * @param desktop     the parent desktop.
	 */
	public RecordSearchInternalFrame ( XmlInformation[] infos, CatalogStar target_star, BaseDesktop desktop ) {
		this.target_star = target_star;
		this.desktop = desktop;

		pane = getContentPane();

		table = new InformationAndPositionTable(target_star.getCoor());
		operation = new RecordSearchOperation(table, target_star, desktop.getFileManager());
		control_panel = new RecordSearchControlPanel(operation, table);
...
//		initMenu();
	 ...
	 */
	protected void setOperation ( RecordSearchOperation operation ) {
